...
HTTP 引擎恢复记录时只装载持久化任务，不使用保存的 URL 发起请求。内存任务缺失而数据库记录仍在时，
`HttpDownloader.restoreState` 保留分片、请求信息和完成状态，并把中断的活动任务置为暂停；已有任务不被覆盖。
存储层报告恢复记录后调用 `resume`，与用户手动继续/重试走相同路径。
可刷新能力先通过 `prepare` 获取访问地址和请求头，再由 `refreshRequest` 校验内容身份与分片布局，最后继续传输。
来源离线或身份校验失败时保留下载记录和已有分片，不回退到过期地址。完成副本直接读取本地文件，不依赖来源能力或登录状态。

动态网页与 BT 加速解析的链接需要刷新。缺少可信内容版本的历史部分下载不能复用已经完成的分片，
需要重新创建下载；URL、标题、资源 ID 和大小不能单独证明内容相同。静态 HTTP 地址沿用普通继续传输行为。
...
HTTP 分段或 HLS 密钥返回 401、403、410 时，下载器报告 `HTTP_ACCESS_EXPIRED`，停止使用同一访问凭证的分段重试。
每次明确创建或继续下载，HTTP 引擎最多执行一次自动能力刷新；第二次拒绝访问保留失败状态供用户处理。
刷新等待旧写入任务关闭后，通过相同内容版本、长度和分段布局检查复用数据。暂停、关闭和删除会取消正在进行的自动刷新。
其他传输错误沿用分段重试策略，来源能力和下载服务之间只传递访问能力与错误类型。
...
1. `Preparing`：从资源库使用的完整条目元数据取得条目与剧集信息，包括用户隐藏的 SP，并检查本条目已有的下载。
...
候选列表、直接选择和提交持久化都要求至少一个已注册下载引擎支持该资源；判断依赖能力接口，来源类型不决定是否可下载。
