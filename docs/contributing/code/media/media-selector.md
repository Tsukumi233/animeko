...
3. **资源类型**：本地缓存永远最前；其余按用户偏好的类型（`preferKind`，在线、BT、本地文件、网络文件或网盘）排序；
...
模糊匹配仍需等待第二截止时间。播放失败换源遵循类型偏好并允许其他来源回退；偏好 Web 时两个截止时间均为 1 秒，
...
- 先选本地缓存，未命中时等待缓存查询完成；然后才考虑已完成的记忆来源及 BT。
- 等偏好类型的启用源全部完成后，按原有偏好规则选择；没有匹配候选时继续等待其他尚未完成的来源，全部完成仍无候选时结束。
- 偏好类型没有启用的源，或没有类型偏好时，等待所有源完成。
- 记忆来源的查询优先完成；未命中后等待偏好类型完成，不使用 WEB 的两段超时。
...
所有非缓存来源的手动选择都保存当前番剧的来源偏好，自动选择及错误回退不写此偏好。来源顺序、类型偏好和下载代价共同参与现有排序；关联本身不提供额外排序优先级。

资源的 association 保存准确的条目和剧集身份。选择器按 ID 检查归属，已确认的资源不经过文件名、续集名称或 BT 单集隐藏等归属猜测；字幕可见性和播放器兼容性偏好仍然适用。对应其他剧集的资源被排除，同集多个版本保持独立候选。
