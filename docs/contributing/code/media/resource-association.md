...
调用方通过 `ignored` 参数提交跳过的文件，可以只提交跳过决定而不访问条目服务或来源工厂。跳过与关联在同一个事务中保存，同一文件不能同时出现在两者中。跳过不删除已有绑定；明确重新关联文件会清除该文件的跳过决定。

`LibraryIgnoredFiles` 作为版本化 JSON 保存在 `library_match_suggestion.suggestionJson`，`ignored` 表示此记录包含跳过决定。独立视频使用空路径，BT 使用完整文件路径；它不表示整个 BT 发布都被忽略。其他路径的决定在确认和重新扫描时保留。
...
`MediaSourceMediaFetcher` 在条目会话中分别维护自动查询缓存与已确认关联候选。资源库 revision 更新只重读本地关联；解除关联后，同 ID 的自动候选可从查询缓存恢复。合并列表变化不修改 `MediaSelector.selected`，正在播放的视频和观看进度由既有播放生命周期持有。
